...
public final class SzenarioAusfuehrungsService {
    ...
    public boolean fuehreSzenarioAus(final Szenario szenario) {
        ...
        final SimpleDateFormat sdf = new SimpleDateFormat("dd.MM 'um' HH:mm:ss 'Uhr'", Locale.GERMANY);
        for (final Szenario.Aenderung aenderung : szenario.getAenderungen().values()) {
            final Map<String, String> attributeMap = aenderung.geraet().getValues();
